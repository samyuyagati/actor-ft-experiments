...
class CentralizedBatcher:
    def __init__(self, batch_size, batcher_fp, worker_fp, pass_by_value, num_workers, worker_resource, checkpoint):
        ...
        for i in range(num_workers):
            # w = worker_cls.options(resources={worker_resource: 1}).remote(worker_fp) #RRR
            w = worker_cls.remote(worker_fp)
            if not self.workers:
        ...
        self.results = []
        self.removed_worker = None
        # self.extra_latencies = []
    ...
    def request(self, img_refs, request_start):
        ...
        # if self.checkpoint and self.checkpoint_count == 700: #EEE
        #     # for worker in self.workers:
        #     #     worker_pid = ray.get(worker.pid.remote())
        #     #     os.kill(worker_pid, signal.SIGKILL)
        #     tmp_worker = self.workers.pop(0)
        #     self.removed_worker = tmp_worker
        #     self.workers.append(tmp_worker)
        #     worker_pid = ray.get(tmp_worker.pid.remote())
        #     os.kill(worker_pid, signal.SIGKILL)
        #     # # self.checkpoint = False
        #     print("worker failed -- waiting for it to restart")
        #     ray.get(tmp_worker.start.remote())
        #     # # ray.get([worker.start.remote() for worker in self.workers])
        #     print("worker restarted")

        # if self.checkpoint and self.checkpoint_count == 800 and self.removed_worker != None:
        #     self.workers.append(self.removed_worker)
        ...
        if self.checkpoint and self.checkpoint_count % 16 == 0: #EEE
            print("checkpoint count ", self.checkpoint_count)
            self.checkpoint_workers() #CCC

        # # Save requests for restart
        # if self.checkpoint and self.checkpoint_count < 650: #EEE #CCC
        #     self.queue_copy.append((img_refs[0], request_start))

        # # # # Clean up worker state and restart all previous requests
        # if self.checkpoint and self.checkpoint_count == 650: #EEE
        #     # Discard results
        #     res = ray.get(self.predictions)
        ...
        #     ray.get(tmp_worker.start.remote())
        #     print("worker restarted")
        #     # # self.results.append(ray.get(self.predictions))
        #     # # for worker in self.workers:
        #     # #     self.extra_latencies.append(ray.get(worker.get_latencies.remote()))
    ...
    def checkpoint_workers(self):
        self.results.append(ray.get(self.predictions))
        file1 = open("/home/ubuntu/ray_source/experimental/checkpoint.txt", "w") #EEE
        # file1 = open("/Users/accheng/Documents/ray_source/experimental/checkpoint.txt", "w")
        file1.write("checkpoint\n")
        file1.close()
        # time.sleep(10)
    ...
    def get_latencies(self):
        ...
        # Account for restart latencies
        # if self.checkpoint: #CCC
        #     print("latencies len ", len(latencies))
        #     # print("self.extra_latencies len ", len(self.extra_latencies))
        #     # print("self.extra_latencies ", self.extra_latencies)
        #     total_i = 0;
        #     for i in range(len(self.extra_latencies)):
        #         for j in range(len(self.extra_latencies[i])):
        #             if total_i > len(latencies):
        #                 break
        #             # latencies[i] += self.extra_latencies[i][j]
        #             total_i += 1
        #     print("self.extra_latencies len ", total_i)
        return latencies
...
class Client:
    ...
    def __init__(self, batcher, pass_by_value, worker_resource, request_rate, batch_size, global_checkpoint):
        self.batcher = batcher
        self.pass_by_value = pass_by_value
        self.worker_resource = worker_resource
        self.img = load_img('/home/ubuntu/ray_source/experimental/img.jpg', target_size = (224, 224)) #EEE
        # self.img = load_img('/Users/accheng/Documents/ray_source/experimental/img.jpg', target_size = (224, 224)) #RRR
    ...
    def run_concurrent(self, num_batches):
        ...
        for i in range(num_batches):
            ...
            for j in range(self.batch_size):
                ...
                # img_ref = preprocess.options(resources={self.worker_resource: 1}).remote(self.img) #RRR
                img_ref = preprocess.remote(self.img)
                if self.pass_by_value:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--global-checkpoint", action="store_true")
    ...
    # ray.init(address="auto", log_to_driver=False) #RRR
    ...
    #     worker_resources[i % len(worker_resources)], args.checkpoint) for i  in range(args.num_batchers_per_node * args.num_nodes)]
    ...
    #     args.batch_size, args.global_checkpoint) for i in range(args.num_clients_per_node * args.num_nodes)]
    ...
    ray.init(log_to_driver=True) #address="auto", log_to_driver=False)
    if args.centralized:
        batcher_cls = CentralizedBatcher
    else:
        batcher_cls = ray.remote(max_restarts=-1, max_task_retries=-1)(Batcher)
    worker_resources = ["serve:{}".format(i) for i in range(args.num_nodes)]
    batchers = [batcher_cls.remote(
        args.batch_size, args.batcher_fp, args.worker_fp, args.pass_by_value, (args.num_workers_per_node // args.num_batchers_per_node),
        worker_resources[i % len(worker_resources)], args.checkpoint) for i  in range(args.num_batchers_per_node * args.num_nodes)]
    clients = [Client.remote(
        ...
        args.batch_size, args.global_checkpoint) for i in range(args.num_clients_per_node * args.num_nodes)]
    # batcher_pid, worker_pid = ray.get(batchers[-1].pid.remote())
    # print("BATCHER PID", batcher_pid)
    # print("WORKER PID", worker_pid)
